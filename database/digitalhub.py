...
class Digitalhub_Mongodb:
                    ...
                    if matched_db_product:
                        ...
                        for variant in product.variants:
                            ...
                            if matched_db_variant:
                                ...
                                self.add_new_variant(variant, matched_db_product['_id'])

                        # # those variants of products which are in database but not found on site
                        # for db_variant in db_variants:
                        #     self.update_not_found_variant(db_variant)
                    else: 
    ...
    def get_matched_product(self, product: Product, db_products: list[dict]) -> dict:
        ...
        try:
            matched_index = -1
            for index, db_product in enumerate(db_products):
                if product.id == db_product['_id'] and product.type == db_product['type']:
                    matched_index = index
                    break
