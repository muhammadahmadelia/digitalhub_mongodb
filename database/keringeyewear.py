...
class Keringeyewear_Mongodb:
    ...
    def controller(self, store: Store) -> None:
        try:
            ...
            if self.DEBUG: print(f'Exception in Keringeyewear_Mongodb: controller: {e}')
            self.print_logs(f'Exception in Keringeyewear_Mongodb: controller: {e}')
    ...
    def get_matched_variant(self, scraped_variant: Variant, db_variants: list[Variant]) -> Variant:
        ...
        try:
            ...
            if not matched_db_variant:
                if scraped_variant.title == '':
                    for index, db_variant in enumerate(db_variants):
                        if scraped_variant.barcode_or_gtin == db_variant.barcode_or_gtin:
                            matched_index = index
                            break
