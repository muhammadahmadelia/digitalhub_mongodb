...
class Controller:
    ...
    def multiple_stores_controller(self) -> None:
        try:
            ...
            for store in self.get_stores_to_update(stores):
                self.store = store
    ...
    def single_store_controller(self) -> None:
        try:
            # getting all stores from database
            query_processor = Query_Processor(self.DEBUG, self.config_file, '')
            stores = query_processor.get_stores()
            ...
            if self.DEBUG: print(f'Exception in single_store_controller: {e}')
            self.print_logs(f'Exception in single_store_controller: {e}')
    ...
    def get_stores_to_update(self, stores: list[Store]) -> list[Store]:
        ...
        try:
            ...
            if self.DEBUG: print(f'Exception in get_stores_to_update: {e}')
            else: pass
...
try:
    ...
    print('1. Scrape multiple stores\n2. Scrape single store')
    while True:
        try:
            choice = int(input('Choice: '))
            if choice in [1, 2]:
                if choice == 1: Controller(DEBUG, path).multiple_stores_controller()
                elif choice == 2: Controller(DEBUG, path).single_store_controller()
                break
            else: print('Enter 1 or 2 as choice')
        except: print('Enter 1 or 2 as choice')
